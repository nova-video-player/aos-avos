...
struct sonicStreamStruct {
  ...
  float inputPlayTime;
  float processingSpeed; /* Effective speed used for the last output burst. */
...
int sonicFlushStream(sonicStream stream) {
  ...
  /* Retained input can have a blended speed after a runtime change. Padding
     must carry the same play-time budget, otherwise processStreamInput treats
     the added silence as real input with zero duration and accelerates the tail. */
  if (remainingSamples > 0 && stream->inputPlayTime > 0.0f) {
    speed = remainingSamples * stream->samplePeriod / stream->inputPlayTime;
  }
...
int sonicInputSamplesBuffered(sonicStream stream) {
  return stream->numInputSamples;
}

float sonicProcessingSpeed(sonicStream stream) {
...
static int processStreamInput(sonicStream stream) {
  ...
  stream->processingSpeed = localSpeed;
  if (localSpeed > 1.00001 || localSpeed < 0.99999) {
    if (!changeSpeed(stream, localSpeed)) {
      return 0;
    }
  } else {
