...
static inline int _cue_expired(int start, int duration, int now) {
	return duration > 0 && start >= 0 && (int64_t)start + duration <= now;
}
...
serprintf("cannot allocate subtitle frame!\r\n");
	...
	} else if( !s->sub_dec && !_is_raw_text(fmt) ) {
		s->sub_dec = stream_get_new_dec_sub( fmt );
		if( stream_open_sub_dec( s ) ) {
			stream_drop_subtitles( s );
			return;
	...
	for( ;; ) {
		...
		if( _is_raw_text(fmt) ) {
			...
			int ret = s->sub_dec->decode( s->sub_dec, s->sub_buffer.data, s->cdata_sub.size, s->cdata_sub.time, &f );
			s->cdata_sub.valid = 0;
			if( !ret && f && f->data[0] && s->sub_engine ) {
				if( f->duration <= 0 && s->cdata_sub.subtitle_duration > 0 )
	...
	if( replay && have_bitmap ) {
		*s->subtitle_frame = last_bitmap;
		if( last_bitmap.duration > 0 &&
			...
			_feed_bitmap_to_engine( s, s->subtitle_frame );
	}
	s->subtitle_replay = 0;
}
...
DBG serprintf("_get_next_ext_sub: re-feeding external text track after seek/interrupt\r\n");
	...
	} else if( _is_ffdec_bitmap(fmt) && !s->sub_dec ) {
		s->sub_dec = stream_get_new_dec_sub( fmt );
		if( stream_open_sub_dec( s ) ) {
			stream_drop_subtitles( s );
			return;
...
// Callers tear the old track down unlocked first (pause, idle rendezvous,
// decoder/engine teardown), which can take a while; a full discovery rebuild can
// run entirely inside that window and shrink av.subs_max or replace whatever's at
// sub_stream with a different file. A bare bounds re-check isn't enough to catch
// the second case, so for an external track we re-resolve by path -- same
// continuity logic _stream_check_subtitles_sync() already uses for ITS prev-track
// lookup -- rather than trusting the index captured earlier. Internal tracks skip
// this: their indices are never touched by an external rebuild, so the bound alone
// is sufficient. If the track can no longer be found, nothing is committed and 0 is
// returned -- committing a stale/wrong index and bumping the generation over it
// would also wrongly tell _stream_check_subtitles_sync()'s reselect to defer to a
// selection that was never actually valid.
static int _commit_subtitle_selection( STREAM *s, int sub_stream, int req_ext, const char *req_path )
{
...
// Subtitle-only track switch: internal tracks rebuild only their parser queue.
// Neither internal nor external tracks need to seek or restart audio/video.
static int _switch_subtitle( STREAM *s, int sub_stream, int req_ext, const char *req_path )
{
	...
	int failed = 0;
	if( !_commit_subtitle_selection( s, sub_stream, req_ext, req_path ) ) {
DBGS serprintf("subtitle switch: requested track %d no longer present after a concurrent rescan -- keeping current track\r\n", sub_stream );
		failed = 1;
	} else {
		if( !s->subtitle->ext )
...
serprintf("SsS: not open!\r\n");
	...
	out_of_range = ( sub_stream < 0 || sub_stream >= s->av.subs_max || !s->av.sub[sub_stream].valid );
	if( !out_of_range ) {
...
//		return 0;
	...
	if( s->av.sub[sub_stream].ext || s->parser->reset_subtitle )
		return _switch_subtitle(s, sub_stream, req_ext, req_path);
	...
	int found = _commit_subtitle_selection( s, sub_stream, req_ext, req_path );
