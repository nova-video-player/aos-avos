...
extern int libavos_pcm_channel_mask_supported(int mask);
...
DBGS serprintf("stream_open_audio_dec: clearing request_channels for AC3 recoding\r\n");
	...
	if( !passthrough_mode && !ac3_recoding ) {
		...
		if( s->audio->channels == 7 ) {
			...
			int should_downmix_61 = (supported == 0 || supported == -1);
#else
			int should_downmix_61 = 0;
#endif
			if( should_downmix_61 ) {
				if( desired == 0 || desired >= 7 ) {
					desired = 6;
DBGS				serprintf("stream_open_audio_dec: 6.1 PCM is unreliable, fallback to 5.1\n");
				}
			}
		}
